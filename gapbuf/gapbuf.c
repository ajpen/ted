...
/*
 * TODO: Malloc'd memory likely has garbage. The gap can move in the buffer.
 * How do I differentiate between the garbage values and the actual string?
 *
 * At the start, the gap should be the same size as the buffer. Only when characters
 * are inserted, does the gap is less than the buffer size.
 *
 * Additionally, moving should only happen between the inserted text.
 *
 * I need to revisit this data structure.
 * */
gapbuffer* CreateGapBuffer(int buffersize, int gapsize, int gaploc){

    if (buffersize <= 0 || gapsize <= 0 || buffersize < gapsize){
        buffersize = DEFAULTGAPBUFFERSIZE;
